...
export default function JobPost({ companyLogo, companyName, role, employmenType, location, postedTime, index }) {
  ...
  return (
    <div className='w-full mb-8 pt-3 lg:pb-3 pb-7 px-2 grid grid-cols-[100px,1fr] gap-4 bg-white rounded-md shadow hover:cursor-pointer' onClick={() => navigate(`post/${index}`)}>
      ...
      <div className='w-full grid grid-rows-[16px,1fr,34px] gap-2'>
        <h6 className='text-xs lg:text-sm font-roboto font-bold text-secondaryBlue truncate'>{companyName}</h6>
        <h3 className='lg:text-lg font-roboto font-normal text-secondaryBlue'>{role}</h3>
        <div className='lg:flex items-center justify-between'>
          <span className='text-xs font-roboto font-bold text-secondaryBlue tracking-tight border-1 border-secondaryBlue rounded-md w-16 h-7 text-center flex items-center justify-center'>
            {employmenTypeFormated(employmenType)}
          </span>
          <div className='flex items-center gap-6 lg:px-2 mt-2 lg:mt-0'>
            <div className='flex items-center gap-1 lg:gap-2 lg:px-2'>
